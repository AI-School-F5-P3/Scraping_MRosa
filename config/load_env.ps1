...
"@
    ...
    if (Test-Path $initFilePath) {
        ...
        try {
            # Ejecutar el comando psql y capturar la salida y el código de salida
            psql -U $env:DB_USER -d $databaseName -f $initFilePath
            $output = "psql -U $env:DB_USER -d $databaseName -f $initFilePath"
            ...
            if ($exitCode -eq 0) {
                ...
                Write-Host "`n$SMILE La base de datos se ha inicializado correctamente $CELEBRATION `n`n`n" -ForegroundColor Green
            } else {
                throw "El comando psql terminó con el código de salida $exitCode. Salida: $output"
            }
        }
        catch [Exception] {
            # Manejar excepciones específicas de la ejecución de comandos psql
